...
const getAccessToken = async () => {
    const { data } = await axios_1.default.get("https://graph.facebook.com/v16.0/oauth/access_token", {
        params: {
...
const getProfile = async (id, token) => {
    try {
        ...
        console.log(error);
        console.log(id);
        throw new AppError_1.default("ERR_FETCHING_FB_USER_PROFILE_2");
    }
};
...
const getPageProfile = async (id, token) => {
    try {
        ...
        console.log(error);
        throw new AppError_1.default("ERR_FETCHING_FB_PAGES");
    }
};
exports.getPageProfile = getPageProfile;
const profilePsid = async (id, token) => {
    try {
        const { data } = await axios_1.default.get(`https://graph.facebook.com/v16.0/${id}?access_token=${token}`);
        return data;
    }
    catch (error) {
        console.log(error);
...
const subscribeApp = async (id, token) => {
    try {
        const { data } = await axios_1.default.post(`https://graph.facebook.com/v16.0/${id}/subscribed_apps?access_token=${token}`, {
            subscribed_fields: [
                "messages",
                "messaging_postbacks",
                "message_deliveries",
        ...
        throw new AppError_1.default("ERR_SUBSCRIBING_PAGE_TO_MESSAGE_WEBHOOKS");
    }
};
exports.subscribeApp = subscribeApp;
const unsubscribeApp = async (id, token) => {
    try {
        const { data } = await axios_1.default.delete(`https://graph.facebook.com/v16.0/${id}/subscribed_apps?access_token=${token}`);
        return data;
    }
    catch (error) {
        throw new AppError_1.default("ERR_UNSUBSCRIBING_PAGE_TO_MESSAGE_WEBHOOKS");
    }
};
...
const getSubscribedApps = async (id, token) => {
    try {
        ...
        throw new AppError_1.default("ERR_GETTING_SUBSCRIBED_APPS");
    }
};
...
const getAccessTokenFromPage = async (token) => {
    try {
        ...
        console.log(error);
        throw new AppError_1.default("ERR_FETCHING_FB_USER_TOKEN");
    }
};
exports.getAccessTokenFromPage = getAccessTokenFromPage;
const removeApplcation = async (id, token) => {
    try {
        const { data } = await axios_1.default.delete(`https://graph.facebook.com/v16.0/${id}/permissions`, {
            params: {
                access_token: token
            }
        });
