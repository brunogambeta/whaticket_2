...
const CheckIntegrations_1 = __importDefault(require("../../helpers/CheckIntegrations"));
    ...
    if (showChatBots.options) {
        ...
        const botText = async () => {
            ...
            const optionsBack = options.length > 0
                ...
                : options;
            if (options.length > 0 && choosenQueue.optionType !== 'file') {
                const body = `\u200e${choosenQueue.greetingMessage}\n\n${optionsBack}\n*[ # ]* - Voltar Menu Inicial`;
                const sendOption = await sendMessage(wbot, contact, ticket, body);
                return sendOption;
            }
            if (choosenQueue.optionType === 'file') {
                let mime = '';
                if (choosenQueue.greetingMessage.toLocaleLowerCase().includes('.pdf')) {
                    mime = 'application/pdf';
                }
                else if (choosenQueue.greetingMessage.toLocaleLowerCase().includes('.jpg') || choosenQueue.greetingMessage.toLocaleLowerCase().includes('.jpeg')) {
                    mime = 'image/jpeg'; //image/jpeg
                }
                else if (choosenQueue.greetingMessage.toLocaleLowerCase().includes('.png')) {
                    mime = 'image/png';
                }
                else if (choosenQueue.greetingMessage.toLocaleLowerCase().includes('.gif')) {
                    mime = 'image/gif';
                }
                else {
                ...
                if (mime.includes('image')) {
                    optionsMessage = {
                        image: fs.readFileSync(upload_1.publicFolder + '/company' + ticket.companyId + '/' + choosenQueue.greetingMessage),
                        caption: `*[ # ]* - Voltar Menu Inicial`,
                        fileName: choosenQueue.greetingMessage.toLocaleLowerCase(),
                        mimetype: mime
                    };
                }
                else {
                    optionsMessage = {
                        document: fs.readFileSync(upload_1.publicFolder + '/company' + ticket.companyId + '/' + choosenQueue.greetingMessage),
                        caption: `*[ # ]* - Voltar Menu Inicial`,
                        fileName: choosenQueue.greetingMessage.toLocaleLowerCase(),
                        mimetype: mime
                    };
                }
                const send = await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, { ...optionsMessage });
            }
            else {
                const body = `\u200e${choosenQueue.greetingMessage}\n*[ # ]* - Voltar Menu Inicial`;
                const send = await sendMessage(wbot, contact, ticket, body);
...
async function sendMsgAndCloseTicket(wbot, contact, ticket) {
    const bodyfinaliza = `Estamos terminando esta conversa! Se ainda precisa falar com nosso time, chame novamente!`;
    await sleep(2000);
...
const sayChatbot = async (queueId, wbot, ticket, contact, msg) => {
    ...
    const queues = await (0, ShowQueueService_1.default)(queueId, ticket.companyId);
    let enabledIntegrationActive;
    if (queues.name.toLocaleLowerCase().match(/^.*Financeiro$/) ||
        ...
        let siprovUserLogin;
        let siprovPassLogin;
        let siprovFoneContact;
        let siprovInitialCurrentMonth;
        let siprovFinalCurrentMonth;
        // HINOVA
        let hinovaActive;
        let hinovaToken;
        let hinovaFoneContact;
        let hinovaInitialCurrentMonth;
        let hinovaFinalCurrentMonth;
        // MIKWEB
        let mikwebActive;
        let mikwebToken;
        let mikwebFoneContact;
        let mikwebInitialCurrentMonth;
        let mikwebFinalCurrentMonth;
        // ASAAS
        let asaasActive;
        let asaasToken;
        // BLING
        let blingActive;
        let blingToken;
        let blingInitialCurrentMonth;
        let blingFinalCurrentMonth;
        const enabledIntegration = await (0, CheckIntegrations_1.default)("enabledIntegration", ticket.companyId);
        enabledIntegrationActive = enabledIntegration.isActive;
        if (enabledIntegrationActive) {
            const siprov = await (0, CheckIntegrations_1.default)("siprov", ticket.companyId);
            ...
            const hinova = await (0, CheckIntegrations_1.default)("hinova", ticket.companyId);
            ...
            const mikweb = await (0, CheckIntegrations_1.default)("mikweb", ticket.companyId);
            ...
            const asaas = await (0, CheckIntegrations_1.default)("asaas", ticket.companyId);
            asaasActive = asaas.isActive;
            asaasToken = asaas.token;
            const bling = await (0, CheckIntegrations_1.default)("bling", ticket.companyId);
                        ...
                        axios(config)
                            ...
                            for (var i = 0; i < qtd; i++) {
                                ...
                                for (var y = 0; y < qtdVeiculo; y++) {
                                    ...
                                    const bodyfinaliza = `Estamos terminando esta conversa! Se nuevo!`;
                                    await sleep(2000);
                                    await sendMessage(wbot, contact, ticket, bodyfinaliza);
                                    const ticketUpdateAgent = {
                                        ticketData: {
