...
const processAudio = async (audio, companyId) => {
    console.log("USERXX", companyId, "RE", express_1.request);
    ...
    return new Promise((resolve, reject) => {
        (0, child_process_1.exec)(`${ffmpeg_1.default.path} -i ${audio} -vn -ab 128k -ar 44100 -f ipod ${outputAudio} -y`, (error, _stdout, _stderr) => {
            if (error)
                reject(error);
            if (fs_1.default.existsSync(outputAudio) && fs_1.default.existsSync(audio)) {
                fs_1.default.unlinkSync(audio);
                fs_1.default.renameSync(outputAudio, audio);
            }
            resolve(audio);
        });
    });
...
const processAudioFile = async (audio, companyId) => {
    console.log("USERXXY", express_1.request);
    const outputAudio = `${publicFolder}/company${companyId}/${new Date().getTime()}.mp3`;
    return new Promise((resolve, reject) => {
        (0, child_process_1.exec)(`${ffmpeg_1.default.path} -i ${audio} -vn -ar 44100 -ac 2 -b:a 192k ${outputAudio.replace("companyundefined", `company${companyId}`)} -y`, (error, _stdout, _stderr) => {
            if (error)
                reject(error);
            fs_1.default.unlinkSync(audio);
            resolve(outputAudio);
...
const getMessageOptions = async (fileName, pathMedia, companyId) => {
        ...
        if (typeMessage === "video") {
            ...
            if (typeAudio) {
                options = {
                    ...
                    mimetype: typeAudio ? "audio/mp3" : mimeType,
                    ptt: true
                };
            }
            else {
    ...
    try {
        ...
        if (typeMessage === "video") {
            ...
            if (typeAudio) {
                ...
                options = {
                    audio: fs_1.default.readFileSync(convert),
                    mimetype: typeAudio ? "audio/mp3" : media.mimetype,
                    ptt: true
                };
            }
            else {
