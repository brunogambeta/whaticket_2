...
const TicketTaskMoveToQueue_1 = require("./services/TicketServices/TicketTaskMoveToQueue");
...
async function handleVerifySchedules(job) {
    logger_1.logger.info(`Verificacao de novas mensagem de disparo agendado`);
    try {
    ...
    try {
        ...
        if (campaign.confirmation && campaignShipping.confirmation === null) {
            ...
            if (campaign.mediaPath) {
                const companyId = campaign.companyId; // Obtenha o ID da empresa
...
const handleSendMessagesToQueue = async () => {
    const job = new CronJob('0 * * * * *', async () => {
        await (0, TicketTaskMoveToQueue_1.TicketTaskMoveToQueue)();
    });
    job.start();
};
